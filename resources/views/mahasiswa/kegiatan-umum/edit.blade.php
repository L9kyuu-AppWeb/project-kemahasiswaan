...
@section('title', 'Edit Kegiatan Umum')
...
<div class="max-w-4xl mx-auto px-4 py-8">
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Edit Kegiatan Umum</h2>
                <p class="text-gray-500 text-sm">Perbarui kegiatan umum Anda</p>
            </div>
        </div>
        ...
        <form action="{{ route('mahasiswa.kegiatan-umum.update', $kegiatanUmum->id) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="space-y-6">
                <!-- Informasi Kegiatan -->
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="kategori" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500" required>
                                ...
                                <option value="{{ $kategori->nama_kategori }}" {{ old('kategori', $kegiatanUmum->kategori) == $kategori->nama_kategori ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-gray-700 font-semibold mb-2">Nama Kegiatan <span class="text-red-500">*</span></label>
                            <input type="text" name="nama_kompetisi" value="{{ old('nama_kompetisi', $kegiatanUmum->nama_kompetisi) }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500" required>
                        </div>
                        <div>
                            <label class="block text-gray-700 font-semibold mb-2">Penyelenggara <span class="text-red-500">*</span></label>
                            <input type="text" name="penyelenggara" value="{{ old('penyelenggara', $kegiatanUmum->penyelenggara) }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500" required>
                        </div>
                        <div>
                            <label class="block text-gray-700 font-semibold mb-2">URL Kegiatan</label>
                            <input type="url" name="url_kegiatan" value="{{ old('url_kegiatan', $kegiatanUmum->url_kegiatan) }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500">
                        </div>
                    </div>
                </div>
                ...
                <!-- Data Kegiatan -->
                <div>
                    <h3 class="font-bold text-gray-800 mb-4 border-b pb-2"><i class="fas fa-clipboard-list text-purple-600 mr-2"></i>Data Kegiatan</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="jenis_kegiatan_id" id="jenis_kegiatan_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500" onchange="loadDetails()">
                                ...
                                <option value="{{ $jenis->id }}" {{ old('jenis_kegiatan_id', $kegiatanUmum->jenis_kegiatan_id) == $jenis->id ? 'selected' : '' }}>{{ $jenis->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="detail_kegiatan_id" id="detail_kegiatan_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500" onchange="loadNilai()">
                                ...
                                @if($kegiatanUmum->detailKegiatan)
                                <option value="{{ $kegiatanUmum->detail_kegiatan_id }}" selected>{{ $kegiatanUmum->detailKegiatan->nama }}</option>
                                @endif
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="ruang_lingkup_id" id="ruang_lingkup_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500" onchange="loadNilai()">
                                ...
                                <option value="1" {{ old('ruang_lingkup_id', $kegiatanUmum->ruang_lingkup_id) == 1 ? 'selected' : '' }}>Lokal</option>
                                <option value="2" {{ old('ruang_lingkup_id', $kegiatanUmum->ruang_lingkup_id) == 2 ? 'selected' : '' }}>Kota</option>
                                <option value="3" {{ old('ruang_lingkup_id', $kegiatanUmum->ruang_lingkup_id) == 3 ? 'selected' : '' }}>Provinsi</option>
                                <option value="4" {{ old('ruang_lingkup_id', $kegiatanUmum->ruang_lingkup_id) == 4 ? 'selected' : '' }}>Wilayah</option>
                                <option value="5" {{ old('ruang_lingkup_id', $kegiatanUmum->ruang_lingkup_id) == 5 ? 'selected' : '' }}>Nasional</option>
                                <option value="6" {{ old('ruang_lingkup_id', $kegiatanUmum->ruang_lingkup_id) == 6 ? 'selected' : '' }}>Internasional</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-gray-700 font-semibold mb-2">Nilai/Point</label>
                            <input type="number" name="nilai" id="nilai" value="{{ old('nilai', $kegiatanUmum->nilai ?? 0) }}" readonly class="w-full px-4 py-3 border rounded-lg bg-gray-100 font-bold text-purple-600">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div>
                        <select name="dosen_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500">
                            ...
                            <option value="{{ $dosen->id }}" {{ old('dosen_id', $kegiatanUmum->dosen_id) == $dosen->id ? 'selected' : '' }}>{{ $dosen->nama }} ({{ $dosen->nuptk }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="file" name="dokumen_sertifikat" accept=".pdf" class="w-full px-4 py-3 border rounded-lg">
                            @if($kegiatanUmum->dokumen_sertifikat)
                            <p class="text-xs text-gray-500 mt-1">File saat ini: {{ basename($kegiatanUmum->dokumen_sertifikat) }}</p>
                            @endif
                        </div>
                        <div>
                            <label class="block text-gray-700 font-semibold mb-2">Tanggal Sertifikat</label>
                            <input type="date" name="tanggal_sertifikat" value="{{ old('tanggal_sertifikat', $kegiatanUmum->tanggal_sertifikat?->format('Y-m-d')) }}" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-500">
                        </div>
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">File saat ini: {{ basename($kegiatanUmum->url_surat_tugas) }}</p>
                            @endif
                        </div>
                    </div>
                </div>
