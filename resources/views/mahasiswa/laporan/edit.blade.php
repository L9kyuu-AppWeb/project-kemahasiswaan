...
<head>
    ...
    <title>Edit Laporan Beasiswa - Sistem Kemahasiswaan</title>
    ...
    <div class="max-w-5xl mx-auto px-4 py-8">
        ...
        <div class="bg-gradient-to-r from-yellow-500 to-orange-600 rounded-2xl shadow-lg p-6 mb-6 text-white">
            <h2 class="text-2xl font-bold mb-1">
                ...
                Edit Laporan Beasiswa
            </h2>
            <p class="text-yellow-100 text-sm">Perbarui laporan Anda</p>
        </div>
        ...
        <form action="{{ route('mahasiswa.laporan.update', $laporan->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="tahun_ajar_id" id="tahun_ajar_id" required
                            ...
                            @foreach($tahunAjarList as $ta)
                                <option value="{{ $ta->id }}" {{ old('tahun_ajar_id', $laporan->tahun_ajar_id) == $ta->id ? 'selected' : '' }}>
                                    {{ $ta->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="semester" id="semester" required
                            ...
                            @for($i = 1; $i <= 10; $i++)
                                <option value="{{ $i }}" {{ old('semester', $laporan->semester) == $i ? 'selected' : '' }}>
                                    Semester {{ $i }}
                                </option>
                            @endfor
                        </select>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="sks" id="sks" value="{{ old('sks', $laporan->laporanAkademik->sks ?? '') }}" min="1" max="24"
                    ...
                    <div>
                        ...
                        <input type="number" name="indeks_prestasi" id="indeks_prestasi" value="{{ old('indeks_prestasi', $laporan->laporanAkademik->indeks_prestasi ?? '') }}" 
                ...
                <div class="mt-4">
                    <label for="file_khs" class="block text-gray-700 font-semibold mb-2">
                        ...
                        Upload KHS <span class="text-gray-400 text-sm">(Opsional - kosongkan jika tidak ingin mengubah)</span>
                    </label>
                    ...
                    @if($laporan->laporanAkademik?->file_khs)
                        <p class="text-blue-600 text-xs mt-1"><i class="fas fa-file mr-1"></i>File saat ini: {{ basename($laporan->laporanAkademik->file_khs) }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div id="referal-container" class="space-y-4">
                    @forelse($laporan->laporanReferals as $index => $referal)
                        <div class="referal-item grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
                            <input type="text" name="referal_nama[]" value="{{ $referal->nama }}" placeholder="Nama Referal"
                                   class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                            <input type="text" name="referal_telp[]" value="{{ $referal->no_telp }}" placeholder="No. Telepon"
                                   class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                            <input type="text" name="referal_prodi[]" value="{{ $referal->program_studi }}" placeholder="Program Studi"
                                   class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                        </div>
                    @empty
                        <div class="referal-item grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div id="pendanaan-container" class="space-y-4">
                    @forelse($laporan->laporanPendanaans as $pendanaan)
                        <div class="pendanaan-item p-4 bg-gray-50 rounded-lg space-y-3">
                            <input type="text" name="pendanaan_nama[]" value="{{ $pendanaan->nama_pendanaan }}" placeholder="Nama Pendanaan"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                            <input type="text" name="pendanaan_judul[]" value="{{ $pendanaan->judul }}" placeholder="Judul Kegiatan/Proyek"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <select name="pendanaan_keterangan[]" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                                    <option value="lolos" {{ $pendanaan->keterangan == 'lolos' ? 'selected' : '' }}>Lolos</option>
                                    <option value="tidak" {{ $pendanaan->keterangan == 'tidak' ? 'selected' : '' }}>Tidak Lolos</option>
                                </select>
                                <select name="pendanaan_posisi[]" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                                    <option value="ketua" {{ $pendanaan->posisi == 'ketua' ? 'selected' : '' }}>Ketua</option>
                                    <option value="anggota" {{ $pendanaan->posisi == 'anggota' ? 'selected' : '' }}>Anggota</option>
                                </select>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div id="kompetisi-container" class="space-y-4">
                    @forelse($laporan->laporanKompetisis as $kompetisi)
                        <div class="kompetisi-item p-4 bg-gray-50 rounded-lg space-y-3">
                            <input type="text" name="kompetisi_nama[]" value="{{ $kompetisi->nama_kompetisi }}" placeholder="Nama Kompetisi"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500">
                            <input type="text" name="kompetisi_judul[]" value="{{ $kompetisi->judul }}" placeholder="Judul Karya/Proyek"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <input type="text" name="kompetisi_juara[]" value="{{ $kompetisi->juara }}" placeholder="Juara (1/2/3/Terbaik)"
                                ...
                                <select name="kompetisi_posisi[]" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500">
                                    <option value="ketua" {{ $kompetisi->posisi == 'ketua' ? 'selected' : '' }}>Ketua</option>
                                    <option value="anggota" {{ $kompetisi->posisi == 'anggota' ? 'selected' : '' }}>Anggota</option>
                                </select>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div id="publikasi-container" class="space-y-4">
                    @forelse($laporan->laporanPublikasis as $publikasi)
                        <div class="publikasi-item p-4 bg-gray-50 rounded-lg space-y-3">
                            <input type="text" name="publikasi_judul[]" value="{{ $publikasi->judul }}" placeholder="Judul Publikasi"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                            <input type="text" name="publikasi_tempat[]" value="{{ $publikasi->nama_tempat }}" placeholder="Nama Tempat Publikasi"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <input type="url" name="publikasi_link[]" value="{{ $publikasi->link_jurnal }}" placeholder="Link Jurnal (https://...)"
                                ...
                                <select name="publikasi_kategori[]" class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                                    ...
                                    <option value="sinta1" {{ $publikasi->kategori == 'sinta1' ? 'selected' : '' }}>Sinta 1</option>
                                    <option value="sinta2" {{ $publikasi->kategori == 'sinta2' ? 'selected' : '' }}>Sinta 2</option>
                                    <option value="sinta3" {{ $publikasi->kategori == 'sinta3' ? 'selected' : '' }}>Sinta 3</option>
                                    <option value="sinta4" {{ $publikasi->kategori == 'sinta4' ? 'selected' : '' }}>Sinta 4</option>
                                    <option value="sinta5" {{ $publikasi->kategori == 'sinta5' ? 'selected' : '' }}>Sinta 5</option>
                                    <option value="sinta6" {{ $publikasi->kategori == 'sinta6' ? 'selected' : '' }}>Sinta 6</option>
                                    <option value="garuda" {{ $publikasi->kategori == 'garuda' ? 'selected' : '' }}>Garuda</option>
                                    <option value="q1" {{ $publikasi->kategori == 'q1' ? 'selected' : '' }}>Q1</option>
                                    <option value="q2" {{ $publikasi->kategori == 'q2' ? 'selected' : '' }}>Q2</option>
                                    <option value="q3" {{ $publikasi->kategori == 'q3' ? 'selected' : '' }}>Q3</option>
                                    <option value="q4" {{ $publikasi->kategori == 'q4' ? 'selected' : '' }}>Q4</option>
                                </select>
                            </div>
                        </div>
                    @empty
