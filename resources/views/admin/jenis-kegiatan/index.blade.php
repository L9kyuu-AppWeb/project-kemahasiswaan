    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl shadow-lg p-6 mb-6 text-white">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-indigo-100 text-sm">Kelola jenis kegiatan mahasiswa</p>
                </div>
                <a href="{{ route('admin.master-kegiatan.jenis.create') }}" class="bg-white text-indigo-600 hover:bg-indigo-50 px-6 py-3 rounded-lg transition font-semibold flex items-center gap-2">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.master-kegiatan.jenis.create') }}" class="bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="py-3 px-4">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.master-kegiatan.jenis.edit', $jenis->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1.5 rounded text-xs transition">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <form action="{{ route('admin.master-kegiatan.jenis.destroy', $jenis->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?');">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="3" class="py-8 px-4 text-center text-gray-500">
                                ...
                                <p>Belum ada data jenis kegiatan.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            @if($jenisKegiatans->hasPages())
                <div class="mt-4">{{ $jenisKegiatans->links() }}</div>
            @endif
        </div>
    </div>
@endsection
