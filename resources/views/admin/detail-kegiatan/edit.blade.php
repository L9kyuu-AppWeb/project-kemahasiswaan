    ...
    <div class="max-w-3xl mx-auto px-4 py-8">
        <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h2 class="text-2xl font-bold">Edit Detail Kegiatan</h2>
                </div>
            </div>
            ...
            <form action="{{ route('admin.master-kegiatan.detail.update', $detailKegiatan->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-6">
                    ...
                    <select name="jenis_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500" required>
                        @foreach($jenisKegiatans as $jenis)
                            <option value="{{ $jenis->id }}" {{ old('jenis_id', $detailKegiatan->jenis_id) == $jenis->id ? 'selected' : '' }}>
                                {{ $jenis->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $detailKegiatan->nama) }}"
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.master-kegiatan.detail.index') }}" class="flex-1 bg-gray-200 text-gray-700 px-6 py-3 rounded-lg font-semibold text-center">
                        Batal
                    </a>
                </div>
            </form>
