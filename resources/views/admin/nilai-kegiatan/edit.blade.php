<!DOCTYPE html>
<html lang="id">
<head><meta charset="UTF-8"><title>Edit Nilai Kegiatan</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"></head>
<body class="bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex items-center gap-4 mb-6"><div class="w-12 h-12 bg-gradient-to-br from-yellow-500 to-orange-600 rounded-xl flex items-center justify-center"><i class="fas fa-edit text-white text-xl"></i></div><div><h2 class="text-2xl font-bold">Edit Nilai Kegiatan</h2></div></div>
            ...
            <form action="{{ route('admin.master-kegiatan.nilai.update', $nilaiKegiatan->id) }}" method="POST">@csrf @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <select name="jenis_id" id="jenis_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500" required onchange="loadDetails()">
                            @foreach($jenisKegiatans as $jenis)<option value="{{ $jenis->id }}" {{ old('jenis_id', $nilaiKegiatan->jenis_id) == $jenis->id ? 'selected' : '' }}>{{ $jenis->nama }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="detail_id" id="detail_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500" required>
                            @foreach($detailKegiatans as $detail)<option value="{{ $detail->id }}" {{ old('detail_id', $nilaiKegiatan->detail_id) == $detail->id ? 'selected' : '' }}>{{ $detail->nama }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="ruang_id" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500" required>
                            @foreach($ruangLingkups as $ruang)<option value="{{ $ruang->id }}" {{ old('ruang_id', $nilaiKegiatan->ruang_id) == $ruang->id ? 'selected' : '' }}>{{ $ruang->nama }}</option>@endforeach
                        </select>
                    </div>
                    <div>
