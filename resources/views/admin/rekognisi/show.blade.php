        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            ...
            <div class="flex items-center justify-between mb-6 pb-4 border-b">
                <div class="flex items-center gap-4">
                    <div class="w-14 h-14 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center">
                        <i class="fas fa-certificate text-white text-2xl"></i>
                    </div>
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800">{{ $rekognisi->nama_rekognisi }}</h2>
                        <p class="text-gray-500 text-sm">{{ $rekognisi->jenisRekognisi->nama ?? '-' }}</p>
                    </div>
                </div>
                <div>
                ...
                <div class="bg-gray-50 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-gray-800">{{ $rekognisi->nama_penyelenggara }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Tanggal Sertifikat</span>
                            <span class="font-medium text-gray-800">{{ $rekognisi->tanggal_sertifikat?->format('d M Y') ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-gray-800">{{ $rekognisi->nim }}</span>
                        </div>
                        <div class="flex justify-between">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-gray-50 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <a href="{{ $rekognisi->url_rekognisi }}" target="_blank"
                           class="flex items-center gap-2 text-cyan-600 hover:text-cyan-700">
                            <i class="fas fa-globe"></i>
                            <span>URL Rekognisi</span>
                        </a>
                        @endif
                        @if(!$rekognisi->url_rekognisi)
                ...
                <div class="bg-gray-50 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <a href="{{ Storage::url($rekognisi->dokumen_sertifikat) }}" target="_blank"
                           class="flex items-center gap-2 text-cyan-600 hover:text-cyan-700">
                        ...
                        <a href="{{ Storage::url($rekognisi->foto_kegiatan) }}" target="_blank"
                           class="flex items-center gap-2 text-cyan-600 hover:text-cyan-700">
                        ...
                        <a href="{{ Storage::url($rekognisi->dokumen_bukti) }}" target="_blank"
                           class="flex items-center gap-2 text-cyan-600 hover:text-cyan-700">
                        ...
                        <a href="{{ Storage::url($rekognisi->surat_tugas) }}" target="_blank"
                           class="flex items-center gap-2 text-cyan-600 hover:text-cyan-700">
                            <i class="fas fa-file-alt"></i>
                            <span>Surat Tugas</span>
                        </a>
                        @endif
                        @if(!$rekognisi->dokumen_sertifikat && !$rekognisi->foto_kegiatan && !$rekognisi->dokumen_bukti && !$rekognisi->surat_tugas)
            ...
            <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
                ...
                <p class="text-gray-700">{{ $rekognisi->keterangan_verifikasi }}</p>
            </div>
            @endif
            ...
            <div class="flex gap-3 pt-4 border-t">
                @if($rekognisi->status === 'pending')
                <form action="{{ route('admin.rekognisi.approve', $rekognisi->id) }}" method="POST" class="inline">
                    @csrf
                ...
                <a href="{{ route('admin.rekognisi.index') }}"
    ...
    <div id="rejectModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full mx-4">
            <h3 class="text-xl font-bold text-gray-800 mb-4">
                <i class="fas fa-times-circle text-red-600 mr-2"></i>Tolak Rekognisi
            </h3>
            <form action="{{ route('admin.rekognisi.approve', $rekognisi->id) }}" method="POST">
                @csrf
